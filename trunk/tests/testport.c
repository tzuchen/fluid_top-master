 ...
 int 		t,portnumber;
  ...
  MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
 	  ...
 	  ierr = VecCreate(PETSC_COMM_WORLD,&x);CHKERRQ(ierr);
      	  ierr = VecSetSizes(x,PETSC_DECIDE,40);CHKERRQ(ierr);
	  ...
	  ierr = VecAssemblyEnd(x);CHKERRQ(ierr);
         ...
         // PetscIntView(1,socketviewer,0);

	  //PetscViewerSocketOpen(PETSC_COMM_WORLD,0,PETSC_DECIDE,&socketviewer);
       //  VecView(x, PETSC_VIEWER_SOCKET_WORLD);
