function [Astruc] = maprefine2channel(n,xpi,sol,cdinfo)
%
% 
%
...
tic
 ...
 [xce,yce,D,sm] = streamlinemapr(xc,yc,cdinfo,sol);
 % xce=xc; yce=yc;
 xe       = 0:dx/2:1;
 ye       = 0:dy/2:1;
 ...
 [Xe,Ye,D,sm] = streamlinemapr(Xe,Ye,cdinfo,sol);
 

 Xe = reshape(Xe,2*n+1,2*n+1);  
 Ye = reshape(Ye,2*n+1,2*n+1);
...
w=1;
 ...
 xe1      = reshape(Xe(1:2:end-2,1:2:end-2),n^2,1)+w*1e-5;
 ye1      = reshape(Ye(1:2:end-2,1:2:end-2),n^2,1)+w*1e-5;

 xe2      = reshape(Xe(2:2:end-1,1:2:end-2),n^2,1);
 ye2      = reshape(Ye(2:2:end-1,1:2:end-2),n^2,1)+w*1e-5;

 xe3      = reshape(Xe(3:2:end  ,1:2:end-2),n^2,1)-w*1e-5;
 ye3      = reshape(Ye(3:2:end  ,1:2:end-2),n^2,1)+w*1e-5;

 xe4      = reshape(Xe(1:2:end-2,2:2:end-1),n^2,1)+w*1e-5;
 ye4      = reshape(Ye(1:2:end-2,2:2:end-1),n^2,1);

 xe5      = reshape(Xe(2:2:end-1,2:2:end-1),n^2,1);
 ye5      = reshape(Ye(2:2:end-1,2:2:end-1),n^2,1);

 xe6      = reshape(Xe(3:2:end  ,2:2:end  ),n^2,1)-w*1e-5;
 ye6      = reshape(Ye(3:2:end  ,2:2:end  ),n^2,1);

 xe7      = reshape(Xe(1:2:end-2,3:2:end  ),n^2,1)+w*1e-5;
 ye7      = reshape(Ye(1:2:end-2,3:2:end  ),n^2,1)-w*1e-5;

 xe8      = reshape(Xe(2:2:end-1,3:2:end  ),n^2,1);
 ye8      = reshape(Ye(2:2:end-1,3:2:end  ),n^2,1)-w*1e-5;

 xe9      = reshape(Xe(3:2:end  ,3:2:end  ),n^2,1)-w*1e-5;
 ye9      = reshape(Ye(3:2:end  ,3:2:end  ),n^2,1)-w*1e-5;
...
tic
 for k = 1:nnzA
   ...
   %Av(k)     = min(abs(dx-px)*abs(dy-py)/dx/dy,1);
   %Av(k)     = abs(dx-px)*abs(dy-py);
