%
%  This is to generate x,y,z,all periodical data
% 
...
nproc = 4;
den   = 5;
%X = createcoord(0,2,8*den,1);
%Y = createcoord(0,1,4*den,1);
%Z = createcoord(0,1,4*den,1);

%X = createcoord(0,2,64,1);
%Y = createcoord(0,1,32,1);
%Z = createcoord(0,1,32,1);

X = createcoord(0,2,41,1);
Y = createcoord(0,1,21,1);
Z = createcoord(0,1,21,1);
...
%  alphapos{1} = [0.5   1    0.2    0.5    0.2   0.5];
 ...
 % alphapos{4} = [1    1.5   0.5    0.8    0.2   0.5];
  %alphaind = alphaset(alphapos,cdinfo);
   ...
   alphaind{1} = find(  abs(alphaijk(1,:)/41*21.*alphaijk(2,:)-alphaijk(3,:))<=4 ) 
...
%  cpos{3}  =  [0     2      0.7    1   0.3   0.7   3      1  ];
   ...
   cpos{1}  =  [0     2      0.3   1.3  0.3   0.7   3      1  ]; 
...
% plot the structure
   ...
   % [Y0,Z0] = meshgrid(0.15:dy:0.9,0.15:dy:0.9);
     [Y0,Z0] = meshgrid(0:0.1:1,0:0.1:1);
